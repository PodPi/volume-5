...
U8GLIB_SH1106_128X64 u8g(U8G_I2C_OPT_NONE); // SW SPI Com: SCK = 13, MOSI = 11, CS = 10, A0 = 9, Res = 12
//U8GLIB_SH1106_128X64 u8g(10, 9, 12, 11, 13); // SW SPI Com: SCK = 13, MOSI = 11, CS = 10, A0 = 9, Res = 12
...
int arc=ymax/2;                              
int angle=0;                                    
char* label[] = {"VOLT","PodPi","LABEL3", "VOLT"};      // some custom gauge labels
...
// ------------------------------------------------- void gauge() ------------------------------------------
void gauge(uint8_t angle, int volt) {
...
//  u8g.setFont(u8g_font_fub11);
  ...
  // show scale labels
  u8g.drawStr( 16, 42, "0");                    
  u8g.drawStr( 26, 18, "1");  //25,18
  //u8g.drawStr( 34, 12, "2");  //60,14
  //u8g.drawStr( 60, 18, "3");  //95,18
  u8g.drawStr( 96, 18, "4"); //105,42
  u8g.drawStr( 106, 42, "5"); //105,42
  ...
  if (w<10){                                    // leading 0 when value less than 10
    u8g.print("0");
  } else if (w>99) {                                   // position at 100%
    u8g.setPrintPos(47,60);
  } else {
    u8g.setPrintPos(36,60);
  }
  float v = (float(5)*volt/1023);
  u8g.print(v);
  u8g.print("V");
}
